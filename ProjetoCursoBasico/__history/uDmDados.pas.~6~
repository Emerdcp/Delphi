...
begin
  try
    // não deu certo, teve que usar outra função - fdConexao.Params.DataBase := GetValorIni(ExtractFilePath(Application.ExeName), 'CONFIGURACAO', 'LOCAL');
    fdConexao.Params.Database := GetValorIni(ExtractFilePath(Application.ExeName) + 'config.ini', 'CONFIGURACAO', 'LOCAL_DB');
    fdConexao.Params.LoadFromFile('.\conexao.ini');
